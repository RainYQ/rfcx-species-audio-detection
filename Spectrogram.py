...
if clip_mode == "random_clip_10s":
    for i in range(data_tp.shape[0]):
        ...
        if min_flag:
            y_clip = y[math.floor(data_tp[i][0] * fs - limit + random_data):math.ceil(data_tp[i][1] * fs + random_data)]
            ...
            y_clip = y[math.floor(data_tp[i][0] * fs - random_data):math.ceil(data_tp[i][1] * fs + limit - random_data)]
        ...
        if mode == "save only":
            plt.axis('off')
            plt.savefig("temp/narrowband_spectrogram_" + str(i) + ".png", bbox_inches="tight", pad_inches=0.0)
        if mode == "show":
